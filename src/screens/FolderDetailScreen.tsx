...
import { addProject, deleteProject, loadProjects } from '../storage/projects';
import { addFolder, getFolder, loadSubfolders } from '../storage/folders';
...
export default function FolderDetailScreen({ navigation, route }: Props) {
  ...
  const refresh = useCallback(async () => {
    const [f, sf, ps] = await Promise.all([
      getFolder(folderId),
      loadSubfolders(folderId),
      loadProjects(folderId),
    ]);
    setFolder(f);
    setSubfolders(sf);
    setProjects(ps);
  ...
  const items: DetailItem[] = [
    ...subfolders.map<DetailItem>((f) => ({ kind: 'folder', folder: f, createdAt: f.createdAt })),
    ...projects.map<DetailItem>((p) => ({ kind: 'project', project: p, createdAt: p.createdAt })),
  ...
  const handleCreate = async () => {
    ...
    if (createMode === 'folder') {
      await addFolder(trimmed, folderId);
    } else {
      await addProject(trimmed, folderId);
    }
    closeCreate();
    refresh();
  };

  const handleLongPressFolder = (f: Folder) => {
    Alert.alert(t('home.deleteAlert.title'), t('home.deleteAlert.body', { name: f.name }), [
  ...
  const handleLongPressProject = (p: Project) => {
    Alert.alert(t('folder.deleteAlert.title'), t('folder.deleteAlert.body', { name: p.name }), [
      { text: t('common.cancel'), style: 'cancel' },
      {
        text: t('common.delete'),
        style: 'destructive',
        onPress: async () => {
          ...
          await deletePhotosForProjects([p.id]);
          refresh();
        },
      },
    ]);
    ...
    if (item.kind === 'folder') {
      ...
      return (
        <TouchableOpacity
          style={styles.card}
          activeOpacity={0.75}
          onPress={() => navigation.push('FolderDetail', { folderId: f.id })}
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyTitle}>{t('folder.empty.title')}</Text>
            <Text style={styles.emptyBody}>{t('folder.empty.body')}</Text>
          </View>
        }
      />
